...
message.channel.send(new Discord.MessageEmbed()//Göktürk Software & Mert ☾#6954
...
message.guild.channels.create('🎮・APEX', {type: 'voice'}).then(a => a.setParent(parent.id) && a.setUserLimit(10));
    ...
    message.guild.channels.create('●▬▬▬▬๑「🏆」๑▬▬▬▬●', {type: 'category'}).then(parent => {
message.guild.channels.create('Göktürk Bot #1', {type: 'text'}).then(c => c.setParent(parent.id));
});
});
